...
//获取网卡信息 参数 ifacePrefix 为 ["eth", "em"]
func CoreNetMetrics(ifacePrefix []string) []*model.MetricValue {
	...
	for idx, netIf := range netIfs {
		...
		ret[idx*26+0] = CounterValue("net.if.in.bytes", netIf.InBytes, iface) //总共接受的字节数
		ret[idx*26+1] = CounterValue("net.if.in.packets", netIf.InPackages, iface) //总共接受的包数
		ret[idx*26+2] = CounterValue("net.if.in.errors", netIf.InErrors, iface) //接受的错误数
		ret[idx*26+3] = CounterValue("net.if.in.dropped", netIf.InDropped, iface) //接收的丢包数
		...
		ret[idx*26+8] = CounterValue("net.if.out.bytes", netIf.OutBytes, iface) //上传字节数
		ret[idx*26+9] = CounterValue("net.if.out.packets", netIf.OutPackages, iface) //上传包数
		ret[idx*26+10] = CounterValue("net.if.out.errors", netIf.OutErrors, iface) //上传错误数
		...
		ret[idx*26+16] = CounterValue("net.if.total.bytes", netIf.TotalBytes, iface) //总得字节数
		ret[idx*26+17] = CounterValue("net.if.total.packets", netIf.TotalPackages, iface) //总得包数
